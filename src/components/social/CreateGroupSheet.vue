<template>
  <transition name="fade-up">
    <div v-if="show" class="fixed inset-0 z-[140] bg-slate-900/60 backdrop-blur-md flex flex-col justify-end" @click.self="$emit('close')">
      <div class="bg-white rounded-t-[36px] p-6 max-h-[85vh] flex flex-col shadow-[0_-20px_40px_rgba(0,0,0,0.1)] relative" @click.stop>
        ...
        <input
          ...
          type="text"
          placeholder="给小队起个名字，如：晚饭打卡组"
          class="shrink-0 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 text-[14px] font-medium outline-none focus:border-violet-400 focus:bg-white transition-all mb-4"
        >
        <p class="shrink-0 text-[12px] font-black text-slate-400 mb-2">勾选要拉进来的朋友 (已选 {{ selectedIds.length }})</p>
        <div class="flex-1 overflow-y-auto custom-scroll space-y-2">
          <p v-if="friends.length === 0" class="text-center text-[13px] text-slate-400 font-bold py-8">还没有好友,可以先建空队稍后再邀请</p>
          <div
        ...
        <div class="shrink-0 pt-4 mt-2 border-t border-slate-100">
          <button @click="$emit('create')" class="w-full bg-violet-600 hover:bg-violet-500 text-white font-black text-[15px] py-4 rounded-[20px] shadow-lg active:scale-95 transition-all flex justify-center items-center gap-2">
...
defineEmits(['close', 'update:name', 'toggle-member', 'create'])
</script>
